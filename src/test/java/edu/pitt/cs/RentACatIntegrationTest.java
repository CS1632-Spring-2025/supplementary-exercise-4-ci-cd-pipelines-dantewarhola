...
public class RentACatIntegrationTest {
	...
	public void testGetCatNullNumCats0() {
		try {
            RentACat rentACat = RentACat.createInstance(InstanceType.IMPL);
            Class<?> rentACatClass = rentACat.getClass();
            Method getCatMethod = rentACatClass.getDeclaredMethod("getCat", int.class);
            getCatMethod.setAccessible(true);

            Cat result = (Cat) getCatMethod.invoke(rentACat, 2);
	...
	public void testGetCatNumCats3() {
		try {
			RentACat rentACat = RentACat.createInstance(InstanceType.IMPL);
			Class<?> rentACatClass = rentACat.getClass();
			Method addCatMethod = rentACatClass.getDeclaredMethod("addCat", Cat.class);
			addCatMethod.setAccessible(true);
			addCatMethod.invoke(rentACat, Cat.createInstance(InstanceType.IMPL, 1, "Jennyanydots"));
			addCatMethod.invoke(rentACat, Cat.createInstance(InstanceType.IMPL, 2, "Old Deuteronomy"));
			addCatMethod.invoke(rentACat, Cat.createInstance(InstanceType.IMPL, 3, "Mistoffelees"));
	...
	public void testRenameNumCat3() {
		...
		System.err.println("DEBUG: List of cats before renaming:");
    	System.err.println(r.listCats());
	...
	public void testReturnFailureCatNumCats3() {
		...
		System.err.println("Before renting, available cats: ");
		System.err.println(r.listCats());
		...
		System.err.println("Renting cat 2 result: " + rentResult);
		assertEquals("Expected to return true", true, rentResult);

		out.reset();

		boolean returnResult = r.returnCat(2);
		System.err.println("Returning cat 2 result: "+ returnResult);
		assertEquals(true, rentResult);

		out.reset();

		boolean secondResult = r.returnCat(2);
		System.err.println("Returning cat 2 again result: " + secondResult);
